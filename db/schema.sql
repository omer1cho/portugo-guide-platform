...
create table guides (
  ...
  -- ההפרשה לפורטוגו לראש משלם בסיורי קלאסי. ותיקים: 10€, חדשים מאמצע 2026 ואילך: 11€
  classic_transfer_per_person numeric default 10,
  created_at timestamptz default now()
);

-- Seed current guides — classic_transfer_per_person uses default (10) for veterans, 11 for new joiners
insert into guides (name, city, travel_type, has_vat, has_mgmt_bonus, mgmt_bonus_amount, classic_transfer_per_person) values
  ('אביב', 'lisbon', 'monthly', false, false, 0, 10),
  ('יניב', 'lisbon', 'monthly', false, false, 0, 10),
  ('תום', 'porto', 'monthly', false, false, 0, 10),
  ('מאיה', 'lisbon', 'monthly', true, true, 200, 10),
  ('מני', 'lisbon', 'daily', true, false, 0, 10),
  ('עומר הבן', 'lisbon', 'daily', false, false, 0, 10),
  ('דותן', 'porto', 'daily', false, false, 0, 10),
  ('ניר', 'lisbon', 'monthly', false, false, 0, 11);


-- ───────────────────────────────────────────────────────────────
-- 2. סיורים (Tours) — one row per tour (date + tour_type + guide)
-- ───────────────────────────────────────────────────────────────
create table tours (
  id uuid primary key default gen_random_uuid(),
...
-- ───────────────────────────────────────────────────────────────
-- 3. הזמנות (Bookings) — sub-groups within a tour
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 4. פעילויות (Activities) — אשל, הברזה, הכשרה
-- ───────────────────────────────────────────────────────────────
create table activities (
  id uuid primary key default gen_random_uuid(),
  guide_id uuid not null references guides(id) on delete cascade,
  activity_date date not null,
  activity_type text not null check (activity_type in ('eshel', 'habraza', 'training', 'training_lead', 'external')),
...
-- ───────────────────────────────────────────────────────────────
-- 5. העברות (Transfers) — guide moving money to Portugo
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 6. הוצאות (Expenses) — from the expenses envelope
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 7. קופות (Cash Boxes) — current state per guide
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 8. חודשים סגורים (Closed Months) — once closed, no edits allowed
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 9. תיקונים ידניים (Manual Overrides) — corrections by admin
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- 10. תצוגת סיכום חודשי (Monthly Summary View)
