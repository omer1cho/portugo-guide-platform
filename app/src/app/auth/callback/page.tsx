 ...
 *
 * אנחנו משתמשים ב-Implicit flow (ולא PKCE) כדי שהקישור יעבוד גם
 * כשהמדריך פותח אותו מאפליקציית מייל שמפנה לדפדפן פנימי שונה מזה
 * שבו הוזן המייל. ב-implicit ה-tokens מועברים ב-URL hash, ה-SDK
 * שולף אותם אוטומטית (`detectSessionInUrl: true`).
 *
 * הבלוק של PKCE (?code=) עוד נשמר כ-fallback למקרה שמישהו לוחץ על
 * קישור ישן שנוצר לפני המעבר.
 *
 * אחרי שהסשן נוצר:
