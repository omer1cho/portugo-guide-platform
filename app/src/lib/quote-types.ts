...
export type QuoteTourSel = {
  ...
  extras?: QuoteExtras;            // משתתפים נוספים במחיר מיוחד (למשל מדריכים) — שורה נפרדת בבלוק המחיר
};

/**
 * משתתפים נוספים במחיר מיוחד לסיור: לא נספרים במדרגת המחיר של הקבוצה,
 * מוצגים כשורה נפרדת ("45€ לאדם · 2 מדריכים") ונכנסים לסה"כ. בקשת עומר 4.9.26.
 */
export type QuoteExtras = {
  count: number;
  pricePerPerson: number;
  label: string;   // מי הם (טקסט חופשי של עומר): "מדריכים", "מלווים"...
};
