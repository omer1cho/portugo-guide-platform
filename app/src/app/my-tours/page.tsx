...
function MyToursContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setTours((data as TourWithBookings[]) || []);
      setLoading(false);
    }
    load();
  }, [router, month]);
    ...
    <div className="min-h-screen pb-20 bg-gray-50">
      <header className="bg-green-800 text-white p-4 shadow-md sticky top-0 z-10">
        <div className="max-w-md mx-auto flex justify-between items-center">
          ...
          <h1 className="text-lg font-bold">הסיורים שלי</h1>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto p-4 space-y-3">
        ...
        {loading ? (
          <div className="text-center py-8 text-gray-500">טוען את הסיורים שלך...</div>
        ) : tours.length === 0 ? (
          <div className="text-center py-8 text-gray-500 bg-white rounded-xl shadow">
            עוד לא הוספת סיור בחודש הזה — בוא.י נתחיל! 👇
          </div>
        ) : (
          tours.map((t) => {
