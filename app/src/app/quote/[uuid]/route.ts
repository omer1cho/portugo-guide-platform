...
(function(){
  ...
  // 5.5) בניית הסקציה התחתונה מחדש אחרי הסתרת הסיורים שלא בהצעה: כך הצ'יפים
  //      "להוסיף עוד סיור" משקפים רק את הסיורים שכן נכנסו להצעה, וכשיש סיור
  //      אחד בלבד הסקציה נעלמת לגמרי (אין חלופות להציע). מספר הסיורים נלקח
  //      מההצעה עצמה (sel.tours) ולא מספירת כרטיסים, כי סיור יחיד עשוי להופיע
  //      כמה כרטיסים (למשל קולינרי בוקר/צהריים).
  window.__PROPOSAL_TOUR_COUNT__ = ${sel.tours.length};
